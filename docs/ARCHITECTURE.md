...
soqu-mem trusts the **agent** to decide what's worth remembering — not a firehose of raw tool calls.

### The Agent Saves, soqu-mem Stores
...
```
soqu-mem/
├── cmd/soqu-mem/main.go              # CLI entrypoint
├── internal/
...
├── plugin/
│   ├── opencode/soqu-mem.ts          # OpenCode adapter plugin
...
├── skills/                         # Contributor AI skills (repo-wide standards + soqu-mem-specific guardrails)
...
```
soqu-mem setup [agent]      Install/setup agent integration (opencode, claude-code, gemini-cli, codex)
...
soqu-mem timeline <obs_id>  Chronological context around an observation
soqu-mem context [project]  Recent context from previous sessions
soqu-mem stats              Memory statistics
soqu-mem export [file]      Export all memories to JSON
soqu-mem import <file>      Import memories from JSON
soqu-mem sync               Export new memories as compressed chunk to .soqu-mem/
soqu-mem sync --all         Export ALL projects (ignore directory-based filter)
