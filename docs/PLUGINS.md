...
```bash
# Install via soqu-mem (recommended — works from Homebrew or binary install)
soqu-mem setup opencode

# Or manually: cp plugin/opencode/soqu-mem.ts ~/.config/opencode/plugins/
```

The plugin auto-starts the HTTP server if it's not already running — no manual `soqu-mem serve` needed.
...
The plugin:
- **Auto-starts** the soqu-mem server if not running
- **Auto-imports** git-synced memories from `.soqu-mem/manifest.json` if present in the project
...
# Or via soqu-mem binary (works from Homebrew or binary install)
...
├── .mcp.json                      # Registers soqu-mem MCP server
...
1. Ensures the soqu-mem HTTP server is running
2. Creates a new session via the API
3. Auto-imports git-synced chunks from `.soqu-mem/manifest.json` (if present)
