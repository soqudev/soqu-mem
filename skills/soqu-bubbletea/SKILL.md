...
name: soqu-bubbletea
description: >
  Bubbletea TUI patterns for the soqu-mem terminal UI and related Go TUI code.
  Trigger: When editing Go files in internal/tui/, working on TUI screens, or adding new UI features.
license: Apache-2.0
metadata:
  author: soqudev
  version: "1.0"
...
Use this skill when:
- Adding new screens to the soqu-mem TUI
...
```bash
go build -o soqu-mem ./cmd/soqu-mem   # Build CLI
go test ./internal/tui/...            # Run TUI tests
go test -run TestNewFeature ./internal/tui/...  # Run specific test
```
...
- **Model**: See `internal/tui/model.go` for state management
- **Update**: See `internal/tui/update.go` for input handling
- **View**: See `internal/tui/view.go` for rendering
- **Styles**: See `internal/tui/styles.go` for Lipgloss styles
