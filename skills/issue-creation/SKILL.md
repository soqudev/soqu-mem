---
name: soqu-mem-issue-creation
description: >
  Issue creation workflow for soqu-mem following the issue-first enforcement system.
...
4. **Questions go to [Discussions](https://github.com/soqudev/soqu-mem/discussions)**, not issues
...
| **soqu-mem Version** | Output of `soqu-mem version` |
...
### Steps to Reproduce
1. Start two soqu-mem instances pointing to the same DB
...
### Problem Description
When scripting with soqu-mem, parsing the human-readable output of mem search is fragile. There's no machine-readable output format.

### Proposed Solution
Add a \`--json\` flag to \`soqu-mem mem search\` that outputs results as JSON.

Example:
\`\`\`bash
soqu-mem mem search \"auth middleware\" --json
\`\`\`
