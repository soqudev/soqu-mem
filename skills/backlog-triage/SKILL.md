---
name: soqu-mem-backlog-triage
description: >
  Backlog triage protocol for soqu-mem: audit open issues and PRs, classify each item,
...
Use this skill when:
- Running a full backlog audit on soqu-mem (or any repo)
...
## Maintainer Philosophy (soqu-mem)
...
| **Local-first** | Data lives in `~/.soqu-mem/soqu-mem.db`. No cloud dependency by default. |
| **Single binary** | One `soqu-mem` binary. No daemon, no service, no secondary processes needed. |
...
| **Tight scope** | Reject features that expand soqu-mem's surface area without a compelling case. |
| **Small focused contributions** | Prefer 50-line PRs solving one problem over 500-line PRs solving five. |
| **Reject vague/scope-breaking work** | Close scope-creep issues and PRs that turn soqu-mem into something else. |
...
- **This skill targets soqu-mem's workflow.** Adapt the philosophy table and label system when applying the reusable prompt to a different repository.
