// Package server provides the HTTP API for soqu-mem.
//
...
// by autosync.Manager and injected from cmd/soqu-mem/main.go.
...
func (s *Server) Start() error {
	...
	if err != nil {
		return fmt.Errorf("soqu-mem server: listen %s: %w", addr, err)
	}
	log.Printf("[soqu-mem] HTTP server listening on %s", addr)
...
func (s *Server) handleExport(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Disposition", "attachment; filename=soqu-mem-export.json")
...
func (s *Server) handleMigrateProject(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("[soqu-mem] project migration failed: %v", err)
	...
	log.Printf("[soqu-mem] migrated project %q → %q (obs: %d, sessions: %d, prompts: %d)",
