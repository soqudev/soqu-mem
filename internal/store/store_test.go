...
func TestAddObservationDeduplicatesWithinWindow(t *testing.T) {
	...
	if err := s.CreateSession("s1", "soqu-mem", "/tmp/soqu-mem"); err != nil {
...
func TestScopeFiltersSearchAndContext(t *testing.T) {
	...
	projectResults, err := s.Search("regex", SearchOptions{Project: "soqu-mem", Scope: "project", Limit: 10})
	if err != nil {
	...
	personalResults, err := s.Search("regex", SearchOptions{Project: "soqu-mem", Scope: "personal", Limit: 10})
	if err != nil {
	...
	ctx, err := s.FormatContext("soqu-mem", "personal")
	if err != nil {
...
func TestUpdateAndSoftDeleteExcludedFromSearchAndTimeline(t *testing.T) {
	...
	searchResults, err := s.Search("deleted", SearchOptions{Project: "soqu-mem", Limit: 10})
	if err != nil {
...
func TestDifferentTopicsDoNotReplaceEachOther(t *testing.T) {
	...
	observations, err := s.AllObservations("soqu-mem", "project", 10)
	if err != nil {
...
func TestNewMigratesLegacyObservationIDSchema(t *testing.T) {
	dataDir := t.TempDir()
	dbPath := filepath.Join(dataDir, "soqu-mem.db")
	...
	_, err = db.Exec(`
		...
		INSERT INTO sessions (id, project, directory) VALUES ('s1', 'soqu-mem', '/tmp/soqu-mem');
		INSERT INTO observations (id, session_id, type, title, content, project, created_at)
		VALUES
			(NULL, 's1', 'bugfix', 'legacy null', 'legacy null content', 'soqu-mem', datetime('now')),
			(7, 's1', 'bugfix', 'legacy fixed', 'legacy fixed content', 'soqu-mem', datetime('now')),
			(7, 's1', 'bugfix', 'legacy duplicate', 'legacy duplicate content', 'soqu-mem', datetime('now'));
	`)
	if err != nil {
		_ = db.Close()
	...
	obs, err := s.AllObservations("soqu-mem", "", 20)
	if err != nil {
	...
	results, err := s.Search("legacy", SearchOptions{Project: "soqu-mem", Limit: 10})
	if err != nil {
...
func TestNewMigratesLegacyUserPromptsSyncIDSchema(t *testing.T) {
	...
	_, err = db.Exec(`
		...
		INSERT INTO sessions (id, project, directory) VALUES ('s1', 'soqu-mem', '/tmp/soqu-mem');
		INSERT INTO user_prompts (session_id, content, project) VALUES ('s1', 'legacy prompt', 'soqu-mem');
	`)
	if err != nil {
		_ = db.Close()
		t.Fatalf("seed legacy db: %v", err)
...
`
	...
	obs, err := s.AllObservations("soqu-mem", "", 10)
	if err != nil {
		t.Fatalf("all observations: %v", err)
	}
	if len(obs) != 2 {
...
func TestStatsProjectsOrderedByMostRecentObservation(t *testing.T) {
	...
	if err := s.CreateSession("s2", "soqu-mem", "/tmp/soqu-mem"); err != nil {
...
func TestSessionsOrderedByMostRecentActivity(t *testing.T) {
	...
	_, err := s.db.Exec(
		`INSERT INTO sessions (id, project, directory, started_at) VALUES
		 (?, ?, ?, ?),
		 (?, ?, ?, ?)`,
		"s-older", "soqu-mem", "/tmp/soqu-mem", "2026-02-01 09:00:00",
		"s-newer", "soqu-mem", "/tmp/soqu-mem", "2026-02-02 09:00:00",
	)
	if err != nil {
	...
	_, err = s.db.Exec(
		`INSERT INTO observations (session_id, type, title, content, project, scope, normalized_hash, revision_count, duplicate_count, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, 1, 1, ?, ?)`,
		"s-older", "note", "latest", "session old got new activity", "soqu-mem", "project", hashNormalized("session old got new activity"), "2026-02-03 09:00:00", "2026-02-03 09:00:00",
	)
	if err != nil {
...
func TestSessionObservationsAddPromptImportAndSyncChunks(t *testing.T) {
	...
	promptID, err := s.AddPrompt(AddPromptParams{SessionID: "s1", Content: longPrompt, Project: "soqu-mem"})
	if err != nil {
...
func TestStoreLocalSyncFoundationEnqueuesCoreMutations(t *testing.T) {
	...
	// Enroll "soqu-mem" so mutations are visible via ListPendingSyncMutations.
	if err := s.EnrollProject("soqu-mem"); err != nil {
		t.Fatalf("enroll: %v", err)
	}

	if err := s.CreateSession("sync-session", "soqu-mem", "/tmp/soqu-mem"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestApplyRemoteMutationIdempotent(t *testing.T) {
	...
	create := SyncMutation{
		...
		Op:        SyncOpUpsert,
		Payload:   `{"id":"remote-session","project":"soqu-mem","directory":"/remote"}`,
	}
	...
	obsMutation := SyncMutation{
		...
		Op:        SyncOpUpsert,
		Payload:   `{"sync_id":"obs-remote-1","session_id":"remote-session","type":"decision","title":"Remote","content":"Pulled from cloud","project":"soqu-mem","scope":"project"}`,
	}
...
func TestApplyPulledMutationAcceptsStringifiedSessionPayload(t *testing.T) {
	...
	if session.Project != "soqu-mem" || session.Directory != "/remote" {
...
func TestEndSessionAndTimelineDefaults(t *testing.T) {
	...
	if err := s.CreateSession("s-end", "soqu-mem", "/tmp/soqu-mem"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestStoreAdditionalQueryAndMutationBranches(t *testing.T) {
	...
	if err := s.CreateSession("s-q", "soqu-mem", "/tmp/soqu-mem"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestEndSessionEdgeCases(t *testing.T) {
	...
	if err := s.CreateSession("s-edge", "soqu-mem", "/tmp/soqu-mem"); err != nil {
		t.Fatalf("create session: %v", err)
	}
...
func TestTimelineHandlesMissingSessionRecord(t *testing.T) {
	...
	res, err := s.db.Exec(
		...
		"manual-save", "manual", "orphan", "orphan content", "soqu-mem", "project", hashNormalized("orphan content"),
	)
	if err != nil {
...
func TestNewErrorBranches(t *testing.T) {
	...
	t.Run("fails when db path is a directory", func(t *testing.T) {
		dataDir := t.TempDir()
		dbAsDir := filepath.Join(dataDir, "soqu-mem.db")
	...
	t.Run("legacy migrate surfaces begin and commit hook failures", func(t *testing.T) {
		prepareLegacyStore := func(t *testing.T) *Store {
			...
			if _, err := s.db.Exec(`
				...
				DROP TABLE IF EXISTS observations_fts;
				DROP TABLE observations;
				INSERT OR IGNORE INTO sessions (id, project, directory) VALUES ('s1', 'soqu-mem', '/tmp/soqu-mem');
				CREATE TABLE observations (
					id INT,
					session_id TEXT,
					type TEXT,
				...
				VALUES (1, 's1', 'bugfix', 'legacy', 'legacy row', 'soqu-mem', datetime('now'), datetime('now'));
			`); err != nil {
...
func TestHookFallbacksAndAdditionalBranches(t *testing.T) {
	...
	t.Run("timeline includes before and after in chronological order", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-tl", "soqu-mem", "/tmp/soqu-mem"); err != nil {
			t.Fatalf("create session: %v", err)
		}

		firstID, err := s.AddObservation(AddObservationParams{SessionID: "s-tl", Type: "note", Title: "1", Content: "one", Project: "soqu-mem"})
		if err != nil {
			t.Fatalf("add first observation: %v", err)
		}
		middleID, err := s.AddObservation(AddObservationParams{SessionID: "s-tl", Type: "note", Title: "2", Content: "two", Project: "soqu-mem"})
		if err != nil {
			t.Fatalf("add middle observation: %v", err)
		}
		lastID, err := s.AddObservation(AddObservationParams{SessionID: "s-tl", Type: "note", Title: "3", Content: "three", Project: "soqu-mem"})
		if err != nil {
	...
	t.Run("format context returns specific query stage errors", func(t *testing.T) {
		...
		t.Run("recent observations error", func(t *testing.T) {
			s := newTestStore(t)
			if err := s.CreateSession("s-ctx", "soqu-mem", "/tmp/soqu-mem"); err != nil {
				t.Fatalf("create session: %v", err)
			}
		...
		t.Run("recent prompts error", func(t *testing.T) {
			s := newTestStore(t)
			if err := s.CreateSession("s-ctx2", "soqu-mem", "/tmp/soqu-mem"); err != nil {
				t.Fatalf("create session: %v", err)
			}
...
func TestStoreUncoveredBranchesPushToHundred(t *testing.T) {
	...
	t.Run("add observation, prompt, update forced errors", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-e", "soqu-mem", "/tmp/soqu-mem"); err != nil {
			t.Fatalf("create session: %v", err)
		}

		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-e", Type: "note", Title: "top", Content: "x", Project: "soqu-mem", TopicKey: "x"}); err != nil {
		...
		s.hooks = defaultStoreHooks()
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-e", Type: "note", Title: "dup", Content: "dup content", Project: "soqu-mem"}); err != nil {
		...
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-e", Type: "note", Title: "x", Content: "y", Project: "soqu-mem", TopicKey: "t"}); err == nil {
			t.Fatalf("expected topic query error on closed db")
		}
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-e", Type: "note", Title: "x", Content: "y", Project: "soqu-mem"}); err == nil {
	...
	t.Run("update observation remaining branches", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-u", "soqu-mem", "/tmp/soqu-mem"); err != nil {
			t.Fatalf("create session: %v", err)
		}
		id, err := s.AddObservation(AddObservationParams{SessionID: "s-u", Type: "old", Title: "t", Content: "c", Project: "soqu-mem", TopicKey: "topic/key"})
		if err != nil {
	...
	t.Run("query iterator scan and rows.Err branches", func(t *testing.T) {
		...
		if err := s.CreateSession("s-iter", "soqu-mem", "/tmp/soqu-mem"); err != nil {
			t.Fatalf("create session: %v", err)
		}
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-iter", Type: "note", Title: "one", Content: "one", Project: "soqu-mem"}); err != nil {
			t.Fatalf("add observation: %v", err)
		}
		if _, err := s.AddPrompt(AddPromptParams{SessionID: "s-iter", Content: "prompt", Project: "soqu-mem"}); err != nil {
			t.Fatalf("add prompt: %v", err)
		}
	...
	t.Run("timeline and search type filter branches", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-t2", "soqu-mem", "/tmp/soqu-mem"); err != nil {
			t.Fatalf("create session: %v", err)
		}
		first, _ := s.AddObservation(AddObservationParams{SessionID: "s-t2", Type: "decision", Title: "a", Content: "a", Project: "soqu-mem"})
		_, _ = s.AddObservation(AddObservationParams{SessionID: "s-t2", Type: "decision", Title: "aa", Content: "aa", Project: "soqu-mem"})
		focus, _ := s.AddObservation(AddObservationParams{SessionID: "s-t2", Type: "decision", Title: "b", Content: "b", Project: "soqu-mem"})
		_, _ = s.AddObservation(AddObservationParams{SessionID: "s-t2", Type: "decision", Title: "c", Content: "c", Project: "soqu-mem"})

		if _, err := s.Search("b", SearchOptions{Type: "decision", Project: "soqu-mem", Scope: "project", Limit: 5}); err != nil {
	...
	t.Run("format context and stats remaining branches", func(t *testing.T) {
		s := newTestStore(t)
		if err := s.CreateSession("s-c", "soqu-mem", "/tmp/soqu-mem"); err != nil {
			t.Fatalf("create session: %v", err)
		}
		if _, err := s.AddObservation(AddObservationParams{SessionID: "s-c", Type: "note", Title: "n", Content: "n", Project: "soqu-mem"}); err != nil {
		...
		if _, err := s.FormatContext("soqu-mem", "project"); err == nil {
		...
		s.hooks.queryIt = origQueryIt
		ctx, err := s.FormatContext("soqu-mem", "project")
		if err != nil {
...
func TestEnrollProjectBasic(t *testing.T) {
	...
	if projects[0].Project != "soqu-mem" {
		t.Fatalf("expected project 'soqu-mem', got %q", projects[0].Project)
	}
	...
	enrolled, err := s.IsProjectEnrolled("soqu-mem")
	if err != nil {
...
func TestUnenrollProjectBasic(t *testing.T) {
	...
	// Unenroll.
	if err := s.UnenrollProject("soqu-mem"); err != nil {
