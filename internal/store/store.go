// Package store implements the persistent memory engine for soqu-mem.
//
// It uses SQLite with FTS5 full-text search to store and retrieve
// observations from AI coding sessions. This is the core of soqu-mem —
...
// ExportData is the full serializable dump of the soqu-mem database.
...
func DefaultConfig() (Config, error) {
	...
	if err != nil {
		return Config{}, fmt.Errorf("soqu-mem: determine home directory: %w", err)
	}
	return Config{
		DataDir:              filepath.Join(home, ".soqu-mem"),
...
func New(cfg Config) (*Store, error) {
	if !filepath.IsAbs(cfg.DataDir) {
		return nil, fmt.Errorf("soqu-mem: data directory must be an absolute path, got %q — set SOQU_MEM_DATA_DIR or ensure your home directory is resolvable", cfg.DataDir)
	}
	if err := os.MkdirAll(cfg.DataDir, 0755); err != nil {
		return nil, fmt.Errorf("soqu-mem: create data dir: %w", err)
	}

	dbPath := filepath.Join(cfg.DataDir, "soqu-mem.db")
	db, err := openDB("sqlite", dbPath)
	if err != nil {
		return nil, fmt.Errorf("soqu-mem: open database: %w", err)
	}
	...
	for _, p := range pragmas {
		if _, err := db.Exec(p); err != nil {
			return nil, fmt.Errorf("soqu-mem: pragma %q: %w", p, err)
		}
	}

	s := &Store{db: db, cfg: cfg, hooks: defaultStoreHooks()}
	if err := s.migrate(); err != nil {
		_ = db.Close() // release Windows file lock if tests use TempDir under the same path
		return nil, fmt.Errorf("soqu-mem: migration: %w", err)
	}
	if err := s.repairEnrolledProjectSyncMutations(); err != nil {
		_ = db.Close()
		return nil, fmt.Errorf("soqu-mem: repair enrolled sync journal: %w", err)
	}
