// Package mcp implements the Model Context Protocol server for soqu-mem.
//
// This exposes memory tools via MCP stdio transport so ANY agent
// (OpenCode, Claude Code, Cursor, Windsurf, etc.) can use soqu-mem's
// persistent memory just by adding it as an MCP server.
//
// Tool profiles allow agents to load only the tools they need:
//
//	soqu-mem mcp                    → all 14 tools (default)
//	soqu-mem mcp --tools=agent      → 11 tools agents actually use (per skill files)
//	soqu-mem mcp --tools=admin      → 3 tools for TUI/CLI (delete, stats, timeline)
//	soqu-mem mcp --tools=agent,admin → combine profiles
//	soqu-mem mcp --tools=mem_save,mem_search → individual tool names
package mcp
...
var ProfileAdmin = map[string]bool{
	"mem_delete":   true, // only in OpenCode's SOQU_MEM_TOOLS filter, not in any agent instructions
	"mem_stats":    true, // only in OpenCode's SOQU_MEM_TOOLS filter, not in any agent instructions
	"mem_timeline": true, // only in OpenCode's SOQU_MEM_TOOLS filter, not in any agent instructions
}
...
// when to search for soqu-mem's tools. This string is returned in the
// initialize response and may be added to the system prompt by clients.
const serverInstructions = `soqu-mem provides persistent memory that survives across sessions and context ` +
