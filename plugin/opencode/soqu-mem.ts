/**
 * soqu-mem — OpenCode plugin adapter
 *
 * Thin layer that connects OpenCode's event system to the soqu-mem Go binary.
 * The Go binary runs as a local HTTP server and handles all persistence.
 *
 * Flow:
 *   OpenCode events → this plugin → HTTP calls → soqu-mem serve → SQLite
 *
 * Session resilience:
...
const SOQU_MEM_PORT = parseInt(process.env.SOQU_MEM_PORT ?? "7437")
const SOQU_MEM_URL = `http://127.0.0.1:${SOQU_MEM_PORT}`
const SOQU_MEM_BIN = process.env.SOQU_MEM_BIN ?? "soqu-mem"

// soqu-mem's own MCP tools — don't count these as "tool calls" for session stats
const SOQU_MEM_TOOLS = new Set([
  "mem_search",
  "mem_save",
  "mem_update",
  "mem_delete",
...
const MEMORY_INSTRUCTIONS = `## soqu-mem Persistent Memory — Protocol
...
async function memHTTPFetch(
  path: string,
  opts: { method?: string; body?: any } = {}
): Promise<any> {
  try {
    const res = await fetch(`${SOQU_MEM_URL}${path}`, {
    ...
    // soqu-mem server not running — silently fail
    return null
  }
}

async function isMemServerRunning(): Promise<boolean> {
  try {
    const res = await fetch(`${SOQU_MEM_URL}/health`, {
...
/**
 * Strip <private>...</private> tags before sending to soqu-mem.
...
export const SoquMem: Plugin = async (ctx) => {
  ...
  // Track which sessions we've already ensured exist in soqu-mem
  ...
  // We must not register them as top-level soqu-mem sessions — they cause session
  // inflation (e.g. 170 sessions for 1 real conversation, issue #116).
  const subAgentSessions = new Set<string>()

  /**
   * Ensure a session exists in soqu-mem. Idempotent — calls POST /sessions
  ...
  async function ensureSession(sessionId: string): Promise<void> {
    ...
    // Do not register sub-agent sessions in soqu-mem (issue #116).
    if (subAgentSessions.has(sessionId)) return
    knownSessions.add(sessionId)
    await memHTTPFetch("/sessions", {
      method: "POST",
      body: {
        id: sessionId,
        project,
  ...
  // Try to start soqu-mem server if not running
  const running = await isMemServerRunning()
  if (!running) {
    try {
      Bun.spawn([SOQU_MEM_BIN, "serve"], {
        stdout: "ignore",
        stderr: "ignore",
        stdin: "ignore",
      })
  ...
  if (oldProject !== project) {
    await memHTTPFetch("/projects/migrate", {
      method: "POST",
      body: { old_project: oldProject, new_project: project },
    })
  }

  // Auto-import: if .soqu-mem/manifest.json exists in the project repo,
  // run `soqu-mem sync --import` to load any new chunks into the local DB.
  ...
  try {
    const manifestFile = `${ctx.directory}/.soqu-mem/manifest.json`
    const file = Bun.file(manifestFile)
    if (await file.exists()) {
      Bun.spawn([SOQU_MEM_BIN, "sync", "--import"], {
        cwd: ctx.directory,
        stdout: "ignore",
        stderr: "ignore",
        stdin: "ignore",
  ...
  return {
    ...
    event: async ({ event }) => {
      ...
      if (event.type === "session.created") {
        ...
        // top-level soqu-mem sessions. They cause massive session inflation
    ...
    "tool.execute.after": async (input, output) => {
      if (SOQU_MEM_TOOLS.has(input.tool.toLowerCase())) return
      ...
      if (input.tool === "Task" && output && sessionId) {
        ...
        if (text.length > 50) {
          await memHTTPFetch("/observations/passive", {
            method: "POST",
            body: {
              session_id: sessionId,
    ...
    // This ensures the agent ALWAYS knows about soqu-mem, even after compaction.
    //
    ...
    "experimental.session.compacting": async (input, output) => {
      ...
      // compacted summary to soqu-mem. The new agent reads the compacted
      // summary and this instruction, then saves it as a session summary.
      output.context.push(
        `CRITICAL INSTRUCTION FOR COMPACTED SUMMARY:\n` +
        `The agent has access to soqu-mem persistent memory via MCP tools.\n` +
